...
import { RESPONSIBILITY_STATUS, CHORE_TYPES } from '../lib/types';
...
export default function Dashboard({ house, currentUser, members, chores, onNavigateToChores, onOpenCreateChore, onShowToast }) {
  ...
  // Handlers
  const handleMarkCompleted = (assignmentId, choreTitle) => {
    store.markAssignmentCompleted(assignmentId, currentUser.id);
    onShowToast({ type: 'success', message: `Marked "${choreTitle}" completed!` });
  };
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-16 md:pb-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 font-medium">
            You have <b className="text-indigo-600 dark:text-indigo-400">{myResponsibilitiesCount}</b> responsibilities assigned this week.
          </p>
        </div>

        {/* AVAILABILITY BAR AT TOP OF HOMEPAGE */}
        <div className="flex items-center gap-3">
          <div className="p-3 rounded-2xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-between gap-3 shadow-xs">
            ...
            <button
              ...
              className="text-xs font-extrabold text-indigo-600 dark:text-indigo-400 hover:underline bg-indigo-50 dark:bg-indigo-950/60 px-2.5 py-1 rounded-lg border border-indigo-100 dark:border-indigo-900"
            >
              [Change]
            </button>
          </div>
          ...
          <button
            ...
            className="hidden sm:flex py-3 px-4 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold text-xs shadow-md transition items-center gap-1.5"
          >
      ...
      <div className="grid grid-cols-3 gap-3 sm:gap-4">
        <div className="p-4 rounded-2xl bg-amber-50/80 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 flex flex-col justify-between">
          <span className="text-xs font-bold text-amber-800 dark:text-amber-300">🟠 Pending</span>
          <span className="text-2xl font-extrabold text-amber-900 dark:text-amber-200 mt-1">
            {pendingCount}
          </span>
        </div>

        <div className="p-4 rounded-2xl bg-emerald-50/80 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900/50 flex flex-col justify-between">
          <span className="text-xs font-bold text-emerald-800 dark:text-emerald-300">✓ Completed</span>
          <span className="text-2xl font-extrabold text-emerald-900 dark:text-emerald-200 mt-1">
            {completedCount}
          </span>
        </div>

        <div className="p-4 rounded-2xl bg-rose-50/80 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900/50 flex flex-col justify-between">
          <span className="text-xs font-bold text-rose-800 dark:text-rose-300">🔔 Needs Attention</span>
          <span className="text-2xl font-extrabold text-rose-900 dark:text-rose-200 mt-1">
            {attentionCount}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 pt-2 border-t border-slate-200 dark:border-gray-800">
        ...
        <div className="flex items-center gap-2">
          <div className="inline-flex rounded-xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-1">
            ...
            <button
              ...
              className="px-2 py-0.5 text-xs font-bold text-slate-700 dark:text-slate-300"
            >
              Reset
            </button>
            <button
              ...
              >
                <div className="space-y-3">
                  ...
                  {lastCompletion && (
                    <div className="text-[11px] text-slate-400 italic font-medium">
                      Last completed: {lastCompletion.completed_by_name} —{' '}
                      {new Date(lastCompletion.timestamp).toLocaleTimeString([], {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </div>
                ...
                <div className="pt-2 border-t border-slate-100 dark:border-gray-700 space-y-2">
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => handleMarkCompleted(assignment.id, chore?.title)}
                      className="py-2.5 px-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs transition shadow-xs flex items-center justify-center gap-1.5"
                    >
                    ...
                    <button
                      onClick={() => handleReportAttention(assignment.id, chore?.title)}
                      className="py-2.5 px-3 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-extrabold text-xs transition shadow-xs flex items-center justify-center gap-1.5"
                    >
                  ...
                  <button
                    onClick={() => handleNotifyTeam(assignment.id)}
                    className="w-full py-2 px-3 rounded-xl bg-slate-100 dark:bg-gray-700 hover:bg-slate-200 dark:hover:bg-gray-600 text-slate-700 dark:text-slate-300 font-bold text-xs transition flex items-center justify-center gap-1.5"
                  >
